...
func (p *console) Write(level Level, headerLength int, data []byte) error {
...
func (p *file) Write(level Level, headerLength int, data []byte) error {
	p.mu.Lock()
	defer p.mu.Unlock()
...
func (p *multiFile) Write(level Level, headerLength int, data []byte) error {
